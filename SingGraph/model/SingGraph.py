...
class SSLModel(nn.Module):
    def __init__(self, device):
        ...
        # Hugging Face XLS-R 300M
        self.model = HFWav2Vec2Model.from_pretrained(
            "facebook/wav2vec2-xls-r-300m"
        )

        # MERT
        self.model_mert = AutoModel.from_pretrained(
            "m-a-p/MERT-v1-330M",
            trust_remote_code=True
        )

        # XLS-R 1024 + MERT 1024 = 2048
        self.out_dim = 2048
    ...
    def extract_feat(self, input_data, input_data2):
        ...
        input_tmp2 = input_tmp2.float()

        # [batch, length, dim]
        emb = self.model(input_tmp).last_hidden_state
        emb_mert = self.model_mert(input_tmp2, output_hidden_states=True).last_hidden_state

        # align temporal dimension before concatenation
